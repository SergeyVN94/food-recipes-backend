...
export class RecipeIngredientService {
  ...
  async getIngredients(filter: QueryFilter = {}) {
    ...
    if (query.length > 0) findOptions.where['name'] = Like(query);

    return await this.recipeRepository.find(findOptions) as RecipeIngredient[];
  }
  ...
  async getIngredientsById(id: RecipeIngredient['id']): Promise<RecipeIngredient | null> {
